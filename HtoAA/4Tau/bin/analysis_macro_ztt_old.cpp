...
int main(int argc, char * argv[]) {
  ...
  //const bool applyDRRandomization = cfg.get<bool>("ApplyDRRandomization");
  //const float randomCone = cfg.get<float>("RandomCone");
  ...
  //const string deltaRFileName = cfg.get<string>("DeltaRFileName");
  //TString DeltaRFileName(deltaRFileName);

  //const string deltaRHistName = cfg.get<string>("DeltaRHistName");
  //TString DeltaRHistName(deltaRHistName);

  // sys uncertainties
  const int jetES = cfg.get<int>("JetES"); 
  const int unclusteredES = cfg.get<int>("UnclusteredES"); 
  const bool zPtReweighting = cfg.get<bool>("ZPtReweighting"); 
  const bool topPtReweighting = cfg.get<bool>("TopPtReweighting"); 
  ...
  TH1D * rIsoH = new TH1D("rIsoH","",200,0,2.);
  TH1D * phiIsoH = new TH1D("phiIsoH","",100,0,2*TMath::Pi());
  ...
  TH1D * ptTrkH_2p5to5  = new TH1D("ptTrkH_2p5to5","",200,0,100);
  TH1D * ptTrkH_5to10  = new TH1D("ptTrkH_5to10","",200,0,100);
  TH1D * ptTrkH_7p5to10 = new TH1D("ptTrkH_7p5to10","",200,0,100);
  TH1D * ptTrkH_10to15  = new TH1D("ptTrkH_10to15","",200,0,100);
  TH1D * ptTrkH_15to20  = new TH1D("ptTrkH_15to20","",200,0,100);
  TH1D * ptTrkH_20to25  = new TH1D("ptTrkH_20to25","",200,0,100);
  TH1D * ptTrkH_25to30  = new TH1D("ptTrkH_25to30","",200,0,100);
  TH1D * ptTrkH_20toInf = new TH1D("ptTrkH_20toInf","",200,0,100);
  TH1D * ptTrkH_30toInf = new TH1D("ptTrkH_30toInf","",200,0,100);
  ...
  TH1D * invMassMuTrkH_20to25  = new TH1D("invMassMuTrkH_20to25","",500,0,500);
  TH1D * invMassMuTrkH_25to30  = new TH1D("invMassMuTrkH_25to30","",500,0,500);
  TH1D * invMassMuTrkH_30toInf  = new TH1D("invMassMuTrkH_30toInf","",500,0,500);


  TH1D * invMassMuTrkH_Gt7p5 = new TH1D("invMassMuTrkH_Gt7p5","",500,0,500);
  TH1D * invMassMuTrkH_Gt10 = new TH1D("invMassMuTrkH_Gt10","",500,0,500);
  TH1D * invMassMuTrkH_Gt15 = new TH1D("invMassMuTrkH_Gt15","",500,0,500);
  TH1D * invMassMuTrkH_Gt20 = new TH1D("invMassMuTrkH_Gt20","",500,0,500);
  ...
  TH1D * dzetaH_5to10  = new TH1D("dzetaH_5to10","",200,-100,100);
  ...
  // DeltaR generator
  //TFile * fileDR = new TFile(TString(cmsswBase)+"/src/DesyTauAnalyses/NTupleMaker/data/"+DeltaRFileName);
  //TH1D * histDR  = (TH1D*)fileDR->Get(DeltaRHistName);
  ...
  while (fileList >> filen) {
   ...
   for (int iCand=0; iCand<numberOfCandidates; iCand++) {
	 ...
	 if(zPtReweighting) zptmassweight = correctionWS->function("zpt_weight_nom")->getVal();
       }
     ...
     if (isTOP) { // applying top pT weights
       float topptweight = 1;
       if (topPtReweighting&&topPt>0&&antitopPt>0) topptweight = topPtWeight(topPt,antitopPt,true);
       weight *= topptweight;
     }
	...
	if (applyGoodRunSelection) {
     ...
     for (unsigned int iTrk=0; iTrk<track_count; ++iTrk) {
       ...
       bool isolation = true;
       int nCloseTrks_ = 0;
       float refDirectionEta = trk4.Eta();
       float refDirectionPhi = trk4.Phi();
      /* if (applyDRRandomization) {
	 rand.SetSeed((int)((trk4.Eta()+2.4)*100000));
	 double rannum1  = rand.Rndm();
	 double rannum2  = rand.Rndm();
	 int nProb = 1;
	 double prob[1]; prob[0] = rannum1;
	 double quantile[1];
	 histDR->GetQuantiles(nProb,quantile,prob);
	 float rIso = quantile[0];
	 float phiIso = 2*TMath::Pi()*rannum2;
	 //	 std::cout << "rannum1 = " << rannum1 << "   rannum2 = " << rannum2 << "   rIso = " << rIso << "  phiIso = " << phiIso << std::endl;
	 refDirectionEta = refDirectionEta + rIso*TMath::Cos(phiIso);
	 refDirectionPhi = refDirectionPhi + rIso*TMath::Sin(phiIso);
	 rIsoH->Fill(rIso);
	 phiIsoH->Fill(phiIso);
       }
       else {
	 if (randomCone>0.01) {
	   ...
	   double rannum2  = rand.Rndm();
	   float phiIso = 2*TMath::Pi()*rannum2;
	   refDirectionEta = refDirectionEta + randomCone*TMath::Cos(phiIso);
	   refDirectionPhi = refDirectionPhi + randomCone*TMath::Sin(phiIso);
	 }
       }*/
	 ...
	 float dRtracks = deltaR(refDirectionEta,refDirectionPhi,track_eta[jTrk],track_phi[jTrk]);
	 if (dRtracks<dRIsoMuon) {
       ...
       else if(trackLV.Pt()<7.5) nCloseTrksH_5to10->Fill(nCloseTrks_,weight);
     ...
     if (trackLV.Pt()>7.5) invMassMuTrkH_Gt7p5->Fill(invMassMuTrk,weight);
     if (trackLV.Pt()>10.) invMassMuTrkH_Gt10->Fill(invMassMuTrk,weight);
     if (trackLV.Pt()>15.) invMassMuTrkH_Gt15->Fill(invMassMuTrk,weight);
     if (trackLV.Pt()>20.) invMassMuTrkH_Gt20->Fill(invMassMuTrk,weight);
     if(trackLV.Pt()<5.){
       ptMuonH_2p5to5->Fill(muonLV.Pt(),weight);
       ptTrkH_2p5to5->Fill(trackLV.Pt(),weight);
       ...
       ptTrkH_5to10->Fill(trackLV.Pt(),weight);
       ...
       ptTrkH_10to15->Fill(trackLV.Pt(),weight);
       ...
       ptTrkH_15to20->Fill(trackLV.Pt(),weight);
     ...
     else if(trackLV.Pt()<25.) {
       ptTrkH_20to25->Fill(trackLV.Pt(),weight);
       invMassMuTrkH_20to25->Fill(invMassMuTrk,weight);
     } 
     else if(trackLV.Pt()<30.) {
       ptTrkH_25to30->Fill(trackLV.Pt(),weight);
       invMassMuTrkH_25to30->Fill(invMassMuTrk,weight);
     } 
     if(trackLV.Pt()>20.){
       ptMuonH_20toInf->Fill(muonLV.Pt(),weight);
       ptTrkH_20toInf->Fill(trackLV.Pt(),weight);
     ...
     if(trackLV.Pt()>30.){
       ptTrkH_30toInf->Fill(trackLV.Pt(),weight);
       invMassMuTrkH_30toInf->Fill(invMassMuTrk,weight);
     }
